(function () {
  var fs = require('fs'),
    ...
    fileToLint = process.argv[2],
    jsExtRegExp = /\.js\s*$/,
    filesToLintCmd = 'git diff --name-only',
    gitRootCmd = 'git rev-parse --show-toplevel',
    lintSettings = {
      // Settings below borrowed from https://gist.github.com/haschek/2595796
      // --------------------------------------------------------------------
  ...
  var showLintError = function (error) {
    try {
      if (error.id) {
